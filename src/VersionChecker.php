...
class VersionChecker
{
     ...
     * Constructor
     *
     * @throws Exception if Composer cannot be initialized
     */
    public function __construct()
    {
        if (!defined('HORDE_BASE')) {
            throw new Exception('HORDE_BASE is not defined');
        }
        ...
        if (!file_exists($composerFile)) {
            throw new Exception('Could not find composer.json in ' . $this->rootDir);
        }

        // Load installed.json
        $installedPath = $this->rootDir . '/vendor/composer/installed.json';
        if (!file_exists($installedPath)) {
            throw new Exception('Could not find installed.json at ' . $installedPath);
        }

        $installedData = json_decode(file_get_contents($installedPath), true);
        if ($installedData === null) {
            throw new Exception('Could not parse installed.json at ' . $installedPath);
        }

        $this->installedPackages = $installedData['packages'] ?? $installedData;
        if (!is_array($this->installedPackages)) {
            throw new Exception('Invalid format in installed.json at ' . $installedPath);
        }
        ...
        try {
            ...
            throw new Exception('Failed to initialize Composer: ' . $e->getMessage());
        }
    }
